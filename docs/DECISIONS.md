...
## ADR-015 · Design v2: Vermillon, schwere Schnitte, Reel, Reveals – Demo-Inhalte auf Vorschau-Deployments (15.09.2026)
**Kontext:** Design v1 (Register, Messing, ruhig) hat Pascal nicht überzeugt. Referenz ist sein Relaunch-Prototyp «Delta»: stärkere Schrift, Micro-Animationen, spannendere Struktur, Brand-Video-Reel, Cases prominent. Echte Cases und Bilder liegen noch nicht freigegeben vor; das Design soll trotzdem beurteilbar sein.
**Entscheid:** v2 auf demselben Branch `design/v1`, Spec `docs/design-spec.md` v2. Akzent Vermillon (#d9451c, für kleinen Text #b4360f), Headlines Archivo 740/116, Hero mit maskierten Zeilen, Brand Reel unter dem Hero (scrollgetrieben weitend, nur CSS), Reveals und Count-up über eine einzige Client-Komponente `components/motion/MotionLayer.tsx` (der in CLAUDE.md Regel 1 vorgesehene Motion-Wrapper; rendert nichts, ohne JavaScript bleibt alles sichtbar). Cases als grosse Kacheln 16:10. Neue Zahlenzeile `home.stats` nur mit Briefing-Fakten. Vorschau-Deployments (`VERCEL_ENV=preview`) lesen Inhalte aus dem Mock (Seed-Daten + drei Platzhalter-Cases aus dem Prototyp mit `[PLATZHALTER]`-Marken, Keyvisuals als SVG) statt aus Sanity; `CONTENT_MOCK=0` erzwingt Sanity. Production liest immer Sanity. Kein Code aus dem Prototyp übernommen, nur die Haltung.
**Konsequenzen:** Ein neues `'use client'` (MotionLayer). Reel und Case-Bilder sind Platzhalter, in `docs/PHASES.md` gelistet. Sobald echte Cases im Studio publiziert sind, wird die Vorschau-Weiche entfernt (eine Zeile in `lib/content/index.ts`) und `scripts/mock-cases.ts` bleibt nur für `CONTENT_MOCK=1` lokal.
