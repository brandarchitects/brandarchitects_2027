...
6. **Erstbefüllung und Abgleich – automatisch bei jedem Build.** `scripts/seed.ts` läuft vor jedem Vercel-Build (Token aus der Integration). Es legt fehlende Dokumente an und aktualisiert nur Dokumente, die seit dem letzten Seed **nicht im Studio bearbeitet** wurden. Im Studio bearbeitete Dokumente bekommen nur neue Felder ergänzt, ihre Texte bleiben. Das Build-Log zeigt «✓ seed: N angelegt, N aktualisiert, N nur ergänzt». Es ist nichts zu tun. Nur im Ausnahmefall: Vercel-Variable `SEED_MODE=replace` (alles überschreiben, inkl. Studio-Änderungen) oder `SEED_MODE=off` (Seed aus) – danach wieder entfernen.
